...
public class Widthdrawl extends JFrame implements ActionListener{
	JLabel jhead,jhead1;
	JTextField tamount;
	JButton bwidth,bback,bexit;
	DataUtil d1;
	public Widthdrawl() {
		super("WIDTHDRAWL");
		setLayout(null);
		jhead1=new JLabel("MAXIMUM DAILY WIDTHDRAWL IS RS.10,000");
		jhead=new JLabel("ENTER AMOUNT YOU WANT TO WIDTHDRAWL");
		tamount=new JTextField();
		bwidth=new JButton("WIDTHDRAWL");
		bback=new JButton("BACK");
		bexit=new JButton("EXIT");
		Font f1=new Font("Arial Rounded MT Bold",Font.PLAIN,30);
		
		Font f2=new Font("Arial Rounded MT Bold",Font.PLAIN,25);
		Font f3=new Font("Arial Rounded MT Bold",Font.PLAIN,40);
		jhead1.setFont(f3);
		jhead1.setBounds(250,80,900,35);
		add(jhead1);
		jhead.setFont(f1);
		jhead.setBounds(330,170,850,30);
		add(jhead);
		tamount.setBounds(500,250,300,40);
		add(tamount);
		tamount.setFont(f2);
		bwidth.setBounds(500,340,130,40);
		add(bwidth);
		bback.setBounds(670,340,130,40);
		add(bback);
		bexit.setBounds(530,600,240,40);
		add(bexit);
		bwidth.addActionListener(this);
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==bwidth)
		{
			...
			try{
				...
				n_credit=amt;
				if((Integer.parseInt(value_balance)>amt) && amt<=10000){
				n_balance=Integer.parseInt(value_balance)-amt;
				ArrayList al=new ArrayList<>();
				al.add(value_app);
				al.add(DateFormat.getDateTimeInstance().format(new java.util.Date()));
				al.add("0");
				...
				JOptionPane.showMessageDialog(getContentPane(), "COLLECT YOUR CASH");
				this.dispose();
				new Trasaction();
				...
				al9.add(DateFormat.getDateTimeInstance().format(new java.util.Date()));
				...
				if(amt>10000)
				{
					JOptionPane.showMessageDialog(this, "DAILY WIDTHDRAW LIMIT IS 10000");
				}
			}
	...
	public String emailsend(ArrayList al8) throws UnsupportedEncodingException 
	{
		...
		try{
	          ...
	          msg.setContent("Your account "+String.valueOf(al8.get(0))+ " is debited by INR" +String.valueOf(al8.get(1))+"on"+String.valueOf(al8.get(2))+".Avaliable balance"+String.valueOf(al8.get(3)),"text/plain");
